...
-   **Multi-Lingual Support (NEW):**
    *   **Language Identification:** Can automatically detect the primary spoken language of the video (`en`, `fr`, `ar`, `ary`) as the first step of the pipeline using the `--enable-language-identification` flag. This requires Azure Speech credentials.
    *   **Language-Specific Transcription:** The detected language is used to request transcription in the correct locale from Azure Batch Transcription, improving accuracy.
    *   **Language-Aware Prompting:** The `ActionSummary` analysis dynamically uses system prompts tailored to the detected language, ensuring the LLM generates descriptions and summaries in the source language (e.g., providing a French summary for a French video).
...
```bash
python analyze_video.py my_video.mp4 --output-dir ./my_video_output --use-scene-detection --enable-language-identification
```
...
*   `--overwrite-output`: If set, will overwrite the output directory if it exists. Use with caution.
*   `--enable-language-identification`: **(NEW)** Enable audio language identification at the start of the pipeline. If audio is present, this will detect the language and tailor transcription and analysis prompts accordingly. Requires Azure Speech credentials.
...
##### to test the officeInterview.mp4 file with language identification run the following command (replacing the path with your own):
python analyze_video.py "C:\Users\Admin\Desktop\support partners\Q2\Video Scribe V1\AIR-AI-Models-Cobra-0.1.0\officeInterview.mp4" --fps 1.0 --use-scene-detection --run-async-analyzer --enable-language-identification
