...
export async function POST(req) {
  ...
  const client2 = new SearchClient(
    ...
    // Extract the `messages` from the body of the request
    try{
      const searchResults = await client2.search(messages[messages.length-1].content,{queryType:"semantic",queryLanguage:"en-us",top:5,semanticConfiguration:"sem"})
      let docs=[]
      for await (const result of searchResults.results) {
        docs.push(JSON.stringify(result.document))
      }
