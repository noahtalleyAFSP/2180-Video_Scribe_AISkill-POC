...
import { loginUser, registerUser, uploadVideo, getDataById, overwriteDataById, deleteDataById, findDataByName } from './helpers';
...
export function Player(props) {
  ...
  const handleRegister = async () => {  
    const result = await registerUser(email, password,username,name);  
    if (result.user_id) {  
      setUserId(result.user_id);  
...
const VideoSearch=({res})=>{
  const startFrame = parseInt(res.start_frame);  
  const endFrame = parseInt(res.end_frame); 
  return(
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        <Image
          ...
          src="/placeholder.svg"
          width={40} />
        <span className="text-sm line-clamp-1">{res.summary.substring(0,30)}</span>
      </div>
      <button><PlayIcon className="w-4 h-4" onClick={()=>handleSearchResultClick(startFrame)}/></button>
          ...
          <div className="flex w-full gap-4">
            ...
            <div className="grid w-[300px] gap-2">
              ...
              <Card className="h-4/5">
                ...
                <CardContent>
                  <p className="text-sm">
                  This is a video is a short biopic following the life of a successful Chef, but with a dramatic twist at the end!
                    </p>
                </CardContent>
              </Card>
            </div>
