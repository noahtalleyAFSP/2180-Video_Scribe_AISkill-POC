...
class VideoAnalyzer:
    ...
    TAG_CHUNK_SECONDS: float = 5.0 # Duration for tag analysis chunks
    ...
    def analyze_video(
        ...
        **kwargs,
                ...
                if has_custom_aggregation:
                    ...
                    if copyright_json_str:
                        try:
                            ...
                            print("Warning: Could not parse copyright JSON string for aggregation.")

                    runtime_so_far = time.time() - stopwatch_start_time
                    processed_results = analysis_config.process_segment_results(
                        enriched_segment_results=raw_processed_results if isinstance(raw_processed_results, list) else [],
        ...
        if hasattr(analysis_config, "process_segment_results") and hasattr(analysis_config, "name") and analysis_config.name == "ActionSummary":
            # If the result dict has an 'actionSummary' key, update it with runtime
            if isinstance(final_results, dict) and "actionSummary" in final_results:
                final_results = analysis_config.process_segment_results(
                    enriched_segment_results=processed_results if isinstance(processed_results, list) else [],
                    manifest=self.manifest,
                    env=self.env,
                    parsed_copyright_info=None,
        ...
        for i in range(num_chunks):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if tag_prompt:
                        tag_llm_response = self._call_llm(tag_prompt, log_token_category="tags")
                        # --- EDIT: _parse_llm_json_response returns camelCase keys internally ---
    ...
    async def _analyze_segment_async(
        ...
        async def call_and_parse_chunk_with_retry(chunk_idx, chunk_start, chunk_end, chunk_frames, chunk_times):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    tag_llm_response = await self._call_llm_async(tag_prompt, log_token_category="tags")
                    # --- EDIT: _parse_llm_json_response returns camelCase keys internally ---
                    parsed_chunk = self._parse_llm_json_response(
                         tag_llm_response, expecting_chapters=False, expecting_tags=True
                    ) # Expects {"globalTags": ...}
        ...
        for i in range(num_chunks):
            ...
            chunk_end = min(segment.start_time + ((i + 1) * self.TAG_CHUNK_SECONDS), segment.end_time)
            epsilon = 0.001
            # Get indices of frames within this time chunk
            chunk_indices = [idx for idx, t in enumerate(all_times) if chunk_start - epsilon <= t <= chunk_end + epsilon]
            if not chunk_indices: continue # Skip chunk if no frames fall within its time range

            # --- FIX: Select URLs instead of paths ---
            all_urls = segment.frame_urls if segment.frame_urls else []
            # Check for consistency between paths and URLs before indexing
            if len(all_urls) != len(all_frames):
                 print(f"Warning: Mismatch between frame paths ({len(all_frames)}) and URLs ({len(all_urls)}) for {segment.segment_name}. Cannot select URL subset reliably. Skipping chunk {i+1}.")
                 # Decide on fallback: skip chunk, try paths (might fail size limit), etc.
                 # For now, let's skip the chunk if URLs are inconsistent
                 continue
            # Select the URLs corresponding to the indices for this chunk
            chunk_frames_or_urls = [all_urls[idx] for idx in chunk_indices]
            # --- END FIX ---

            chunk_times = [all_times[idx] for idx in chunk_indices]
            print(f"      - Creating task for Chunk {i+1} ({len(chunk_frames_or_urls)} frames/URLs)")

            # Create a task using the retry wrapper, passing the URLs
            task = asyncio.create_task(
                call_and_parse_chunk_with_retry(i, chunk_start, chunk_end, chunk_frames_or_urls, chunk_times),
    ...
    def _generate_segment_prompt(
        ...
        if generate_chapters and generate_tags:
            ...
            # (Add fallback to defaults if needed, based on _get_list_definitions impl)
            if person_instr == "No specific persons definitions provided.": person_instr = "Identify persons by appearance..."
            if object_instr == "No specific objects definitions provided.": object_instr = "Identify specific physical objects..."
            if action_instr == "No specific actions definitions provided.": action_instr = "Identify noticeable actions/behaviors..."
            ...
            try:
                 ...
                 # Get results template for reminder
                 results_template_obj = getattr(analysis_config, 'results_template_tags', {"globalTags": {"persons":[],"actions":[],"objects":[]}})
    ...
    def _merge_tagging_chunks(self, tag_chunk_results: list[dict]) -> dict:
        """
        Consolidates globalTags coming from many 5‑sec chunks into one
        structure that already contains *merged intervals*:
            {"persons":[{"name":...,"timecodes":[{"start":..,"end":..}, ...]}, ...], ...}
        """
        # 1. Collect raw intervals for every tag
        ...
        for chunk in tag_chunk_results:
            if not isinstance(chunk, dict):
                continue
            gtags = chunk.get("globalTags", {})
            if not isinstance(gtags, dict):
                continue

            for tag_type in ("persons", "actions", "objects"):
                for item in gtags.get(tag_type, []):
                    if not isinstance(item, dict):
                        continue
                    name = str(item.get("name", "")).strip()
                    if not name:
                        continue
                    raw_tc = item.get("timecodes", [])
                    collected[tag_type][name].extend(self._timecodes_to_intervals(raw_tc))

        # 2. Merge overlaps & small gaps
        final_structure = {"persons": [], "actions": [], "objects": []}
        for tag_type, names_dict in collected.items():
            for name, intervals in names_dict.items():
                merged_ints = self._merge_overlapping(intervals, max_gap=2.0)
                final_structure[tag_type].append(
                    {
                        "name": name,
                        "timecodes": [
                            {"start": round(s, 3), "end": round(e, 3)} for s, e in merged_ints
                        ],
                    }
                )
    ...
    def _call_llm(self, messages, images_base64=None, model="gpt-3.5-turbo", log_token_category=None):
        ...
        completion_params = {}
        max_token_value = 3000 # Restore previous max tokens to allow larger completions
