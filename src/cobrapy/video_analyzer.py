...
from .cobra_utils import (
    ...
    # ensure_person_group_exists,  # Removed
    # process_frame_with_faces,    # Removed
...
class VideoAnalyzer:
    ...
    # person_group_id: Optional[str] # Removed
    ...
    def __init__(
        ...
        # --- END ADDED ---
        # person_group_id: Optional[str] = None, # Removed
        ...
        **kwargs,
        ...
        if analysis_config.analysis_sequence == "mapreduce":
            ...
            try:
                ...
                await self._get_async_llm_client()

                raw_processed_results = await self._analyze_segment_list_async(
            ...
            try:
                if hasattr(analysis_config, "run_final_summary") and analysis_config.run_final_summary:
                    ...
                    if should_generate_summary:
                        ...
                        # --- Ensure content_to_summarize is pruned to only chapter-level summaries ---
                        try:
                            chapters_data = None
                            if isinstance(content_to_summarize, dict):
                                chapters_data = content_to_summarize.get("chapters") or content_to_summarize.get("chapter")
                            if isinstance(chapters_data, list):
                                pruned_chapters = []
                                for ch in chapters_data:
                                    if not isinstance(ch, dict):
                                        continue
                                    pruned_chapters.append({
                                        "start": ch.get("start"),
                                        "end": ch.get("end"),
                                        "summary": ch.get("summary") or ch.get("shotDescription") or ""
                                    })
                                content_to_summarize = {
                                    "video_title": content_to_summarize.get("video_title", self.manifest.name) if isinstance(content_to_summarize, dict) else self.manifest.name,
                                    "chapters": pruned_chapters
                                }
                        except Exception as prune_e:
                            logger.warning(f"Failed to prune summary content: {prune_e}")
                        # --- END PRUNE ---
                        ...
                        # --- IF PROMPT TOO LARGE → FALLBACK CHUNKING ---
                        try:
                            serialized_body = json.dumps({"model": self.env.vision.deployment, "messages": summary_prompt_messages})
                            if len(serialized_body) > 900_000 and isinstance(content_to_summarize, dict) and "chapters" in content_to_summarize:
                                logger.info(f"Summary prompt is {len(serialized_body):,} bytes; generating partial summaries to stay within request limits.")
                                chapters_list = content_to_summarize["chapters"]
                                max_chunks = 3  # Number of sub-summaries to generate
                                approx_chunk_size = max(1, len(chapters_list) // max_chunks)
                                partial_summaries = []
                                for chunk_start in range(0, len(chapters_list), approx_chunk_size):
                                    chunk_chapters = chapters_list[chunk_start:chunk_start + approx_chunk_size]
                                    chunk_content = {
                                        "video_title": content_to_summarize.get("video_title", self.manifest.name),
                                        "chapters": chunk_chapters
                                    }
                                    chunk_prompt_messages = [
                                        {"role": "system", "content": formatted_summary_prompt},
                                        {"role": "user", "content": json.dumps(chunk_content)}
                                    ]
                                    try:
                                        chunk_resp = await self._call_llm_async(chunk_prompt_messages, log_token_category="summary_partial")
                                        partial_summaries.append(chunk_resp.choices[0].message.content.strip())
                                    except Exception as chunk_e:
                                        logger.warning(f"Partial summary generation failed: {chunk_e}")
                                if partial_summaries:
                                    merge_prompt = [
                                        {"role": "system", "content": "You are VideoSummaryMergerGPT. Combine the provided partial summaries into a single coherent JSON with keys 'description', 'summary', 'category', and optional 'subCategory'."},
                                        {"role": "user", "content": json.dumps({"partial_summaries": partial_summaries})}
                                    ]
                                    summary_prompt_messages = merge_prompt
                                    logger.info("Using merged partial summaries for final summary generation.")
                        except Exception as chunk_setup_e:
                            logger.warning(f"Chunking setup failed: {chunk_setup_e}")
                        # --- END CHUNKING LOGIC ---
                        ...
                        try:
                            ...
                            summary_results_llm = await self._call_llm_async(summary_prompt_messages, log_token_category="summary") # Renamed variable
            ...
            # --- MODIFIED: Ensure transcription is injected *inside* the actionSummary object ---
            if self.manifest.audio_transcription and isinstance(final_results, dict):
                # Check if 'actionSummary' key exists and is a dictionary
                if "actionSummary" in final_results and isinstance(final_results["actionSummary"], dict):
                    # Inject the transcription into the 'actionSummary' object
                    if "audio_transcription" not in final_results["actionSummary"]:
                        final_results["actionSummary"]["audio_transcription"] = self.manifest.audio_transcription
                        logger.info("Injected full audio transcription into the 'actionSummary' object in the final results.")
                else:
                    # Fallback for other analysis types or if the main structure is different
                    if "audio_transcription" not in final_results:
                        final_results["audio_transcription"] = self.manifest.audio_transcription
                        logger.info("Injected full audio transcription into the top-level final results dictionary ('actionSummary' key not found or not a dict).")
            # --- END MODIFIED ---
    ...
    async def _analyze_segment_async(
        ...
        # --- NEW: Manually inject the segment's transcription into the chapter object ---
        if segment.transcription and "No transcription" not in segment.transcription:
            if "chapters" in final_segment_result and isinstance(final_segment_result["chapters"], list) and len(final_segment_result["chapters"]) > 0:
                final_segment_result["chapters"][0]["transcription"] = segment.transcription
                logger.info(f"[{segment.segment_name}] Injected transcription: {segment.transcription[:80]}...")
            else:
                logger.info(f"[{segment.segment_name}] Could not inject transcription: 'chapters' key not found or empty. LLM result: {final_segment_result}")
        # --- END NEW CODE ---

        # --- NEW: Person/Object detection when YOLO is skipped ---
        persons_detected: list[dict[str, Any]] = []
        objects_detected: list[dict[str, Any]] = []
        if not self.active_yolo_tags_source:  # Only perform if no YOLO tags available (skip-yolo scenario)
            try:
                persons_detected, objects_detected = await self._extract_entities_per_segment_async(segment, analysis_config)
                logger.info(
                    f"Segment {segment.segment_name}: Entity extraction found {len(persons_detected)} persons and {len(objects_detected)} objects."
                )
            except Exception as e:
                logger.error(f"Entity extraction failed for segment {segment.segment_name}: {e}")
        # --- END NEW ---

        # Merge detected persons/objects into final_segment_result (ensure after previous merges)
        if persons_detected or objects_detected:
            if "globalTags" not in final_segment_result or not isinstance(final_segment_result["globalTags"], dict):
                final_segment_result["globalTags"] = {"persons": [], "actions": [], "objects": []}

            # Helper to merge lists without duplicate classDescription entries
            def _merge_tag_lists(existing: list[dict], new: list[dict]) -> list[dict]:
                by_desc = {t.get("classDescription"): t for t in existing if isinstance(t, dict) and t.get("classDescription")}
                for n in new:
                    desc = n.get("classDescription")
                    if not desc:
                        continue
                    if desc in by_desc:
                        # merge timecodes
                        by_desc[desc]["timecodes"].extend(n.get("timecodes", []))
                    else:
                        by_desc[desc] = n
                return list(by_desc.values())

            if persons_detected:
                final_segment_result["globalTags"]["persons"] = _merge_tag_lists(final_segment_result["globalTags"].get("persons", []), persons_detected)
            if objects_detected:
                final_segment_result["globalTags"]["objects"] = _merge_tag_lists(final_segment_result["globalTags"].get("objects", []), objects_detected)

        return final_segment_result

    async def _extract_entities_per_segment_async(self, segment: Segment, analysis_config: Type['AnalysisConfig']) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        """
        When YOLO is skipped, this function sends frames to an LLM to detect persons and objects.
        It now runs person and object detection in parallel.
        """
        logger.info(f"Segment {segment.segment_name}: Running LLM-based entity extraction (YOLO skipped).")
        ...
        if not frames_for_prompt:
            logger.warning(f"Segment {segment.segment_name}: No frames for entity extraction.")
            return [], []

        # --- Prepare Prompts for Parallel Execution ---
        
        # 1. Person Prompt
        person_system_prompt_template = getattr(analysis_config, 'system_prompt_persons', "")
        person_system_prompt = person_system_prompt_template.format(
            person_instructions=self._get_list_definitions(self.peoples_list, "person")
        )
        
        # 2. Object Prompt
        object_system_prompt_template = getattr(analysis_config, 'system_prompt_objects', "")
        object_system_prompt = object_system_prompt_template.format(
            object_instructions=self._get_list_definitions(self.objects_list, "object")
        )

        # Common user content for both prompts
        user_content_parts = []
        frame_timestamps_map = {
            str(i + 1): f"{t:.3f}s"
            for i, t in enumerate(segment.segment_frame_time_intervals)
        }
        
        user_content_parts.append({
            "type": "text",
            "text": "Analyze the following frames to identify entities. Use the provided frame timestamps map for your timecodes."
        })
        user_content_parts.append({
            "type": "text",
            "text": f"frame_timestamps: {json.dumps(frame_timestamps_map)}"
        })

        for i, frame_path_or_url in enumerate(frames_for_prompt):
            user_content_parts.append({"type": "text", "text": f"Image #{i+1}:"})
            if frame_path_or_url.startswith("http"):
                user_content_parts.append({"type": "image_url", "image_url": {"url": frame_path_or_url, "detail": self.image_detail_level}})
            else:
                try:
                    with open(frame_path_or_url, "rb") as image_file:
                        b64_img = base64.b64encode(image_file.read()).decode('utf-8')
                    user_content_parts.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64_img}", "detail": self.image_detail_level}})
                except Exception as e:
                    logger.error(f"Error encoding frame {frame_path_or_url} for entity extraction: {e}")
                    user_content_parts.append({"type": "text", "text": f"[Error reading frame Image #{i+1}]"})

        person_messages = [{"role": "system", "content": person_system_prompt}, {"role": "user", "content": user_content_parts}]
        object_messages = [{"role": "system", "content": object_system_prompt}, {"role": "user", "content": user_content_parts}]

        # --- Run LLM Calls in Parallel ---
        try:
            person_task = self._call_llm_async(person_messages, log_token_category="entity_extraction_person")
            object_task = self._call_llm_async(object_messages, log_token_category="entity_extraction_object")

            responses = await asyncio.gather(person_task, object_task, return_exceptions=True)
            
            person_response, object_response = responses
            
            detected_persons = []
            if isinstance(person_response, Exception) or not hasattr(person_response, 'choices'):
                logger.error(f"LLM call for person detection failed or returned invalid data: {person_response}")
            else:
                person_content = person_response.choices[0].message.content
                parsed_persons = self._parse_llm_json_response(person_content)
                if parsed_persons and "persons" in parsed_persons:
                    detected_persons = parsed_persons["persons"]

            detected_objects = []
            if isinstance(object_response, Exception) or not hasattr(object_response, 'choices'):
                logger.error(f"LLM call for object detection failed or returned invalid data: {object_response}")
            else:
                object_content = object_response.choices[0].message.content
                parsed_objects = self._parse_llm_json_response(object_content)
                if parsed_objects and "objects" in parsed_objects:
                    detected_objects = parsed_objects["objects"]

        except Exception as e:
            logger.error(f"Error during parallel entity extraction for segment {segment.segment_name}: {e}", exc_info=True)
            return [], []

        # --- Post-process and merge timecodes (as was done before) ---
        def _merge_intervals(intervals: list[list[float]], max_gap: float = 1.0) -> list[list[float]]:
        ...
        def _to_timecode_dicts(intervals: list[list[float]]) -> list[dict[str,str]]:
            return [{"start": f"{s:.3f}s", "end": f"{e:.3f}s"} for s, e in intervals]

        all_persons = {}
        for p in detected_persons:
            name = p.get("classDescription")
            if not name: continue
            if name not in all_persons: all_persons[name] = []
            for tc in p.get("timecodes", []):
                try:
                    s = float(str(tc.get("start")).rstrip("s"))
                    e = float(str(tc.get("end")).rstrip("s"))
                    all_persons[name].append([s, e])
                except (ValueError, TypeError): continue

        final_persons = []
        for name, intervals in all_persons.items():
            merged_intervals = _merge_intervals(intervals)
            final_persons.append({"classDescription": name, "timecodes": _to_timecode_dicts(merged_intervals)})

        all_objects = {}
        for o in detected_objects:
            name = o.get("classDescription")
            if not name: continue
            if name not in all_objects: all_objects[name] = []
            for tc in o.get("timecodes", []):
                try:
                    s = float(str(tc.get("start")).rstrip("s"))
                    e = float(str(tc.get("end")).rstrip("s"))
                    all_objects[name].append([s, e])
                except (ValueError, TypeError): continue

        final_objects = []
        for name, intervals in all_objects.items():
            merged_intervals = _merge_intervals(intervals)
            final_objects.append({"classDescription": name, "timecodes": _to_timecode_dicts(merged_intervals)})
            
        logger.info(f"Segment {segment.segment_name}: LLM extraction found {len(final_persons)} persons and {len(final_objects)} objects.")
        return final_persons, final_objects
    ...
    def _generate_segment_prompt(
        ...
        detected_language = self.manifest.source_video.language_code
        if hasattr(analysis_config, 'get_system_prompt'):
            logger.info(f"Using language-aware prompt for language: '{detected_language or 'en'}'")
            system_prompt = analysis_config.get_system_prompt(detected_language)
        else:
            # Fallback for older AnalysisConfig classes
...
"""
    ...
    # --- NEW: System prompt for person & object extraction when YOLO is skipped ---
    SYSTEM_PROMPT_ENTITY_EXTRACTION: ClassVar[str] = """You are EntityDetectorGPT.
Your job is to identify distinct PERSONS (humans) and notable OBJECTS visible in a small batch of video frames.
Return ONLY valid JSON with two keys:
```json
{
  "persons": ["description1", "description2", ...],
  "objects": ["description1", "description2", ...]
}
```
Rules:
1. Descriptions must be concise (3-6 words). Example: "Woman in blue jacket", "Red double-decker bus".
2. Do NOT include duplicate descriptions within the same list.
3. If no persons (or objects) are visible in the batch, return an empty array for that key.
