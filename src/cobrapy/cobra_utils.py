...
# --- CONDITIONALLY IMPORT FACE LIBS ---
try:
    from azure.ai.vision.face import FaceAdministrationClient, FaceClient
    from azure.ai.vision.face.models import FaceDetectionModel, FaceRecognitionModel, QualityForRecognition
    AZURE_AI_VISION_AVAILABLE = True
except ImportError:
    AZURE_AI_VISION_AVAILABLE = False
    # Define dummy classes if the import fails, so the rest of the file can load.
    class FaceAdministrationClient: pass
    class FaceClient: pass
...
def generate_batch_transcript(
    ...
    while time.time() - polling_start_time < max_polling_time:
        try:
            ...
            # ADDED: Print full status from Azure for better debugging
            print(f"DEBUG: Full status data from Azure: {json.dumps(status_data, indent=2)}")
...
def segment_and_extract(
    ...
    output_pattern = os.path.join(frames_dir, "reference_%05d.jpg")
...
def prepare_outputs_directory(
    ...
    *,
    ...
    """
    Prepares the output directory for analysis results.

    If output_directory is provided, it's used as the base.
    If not, a directory is created based on the video file name.

    The final output path will be:
    {output_directory or ""}/{output_directory_prefix}{file_name_sanitized}_{timestamp}

    Args:
        file_name: The base name of the video file.
        output_directory: A specific base directory to save results in.
        overwrite_output: If True, recursively delete the directory if it exists.
        output_directory_prefix: A prefix to add to the generated directory name.

    Returns:
        The absolute path to the created (or cleared) output directory.
    """
    file_name_sanitized = generate_safe_dir_name(Path(file_name).stem)
    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    
    dir_name = (
        f"{output_directory_prefix}"
        f"{file_name_sanitized}_"
        f"{timestamp}"
    )

    if output_directory:
        # If a base output directory is specified, create the unique sub-directory within it.
        final_output_path = Path(output_directory) / dir_name
    else:
        # Otherwise, create it next to the input file.
        final_output_path = Path(file_name).resolve().parent / dir_name

    final_output_path = final_output_path.resolve()

    if final_output_path.exists() and overwrite_output:
        print(f"Output directory {final_output_path} exists. Overwriting.")
        try:
            shutil.rmtree(final_output_path)
        except OSError as e:
            print(f"Error removing directory {final_output_path}: {e}")
            raise
...
def get_face_client(env: CobraEnvironment) -> FaceClient:
    if not AZURE_AI_VISION_AVAILABLE:
        raise RuntimeError("Azure AI Vision SDK is not installed. Cannot create FaceClient.")
    return FaceClient(
        endpoint=env.face.endpoint,
        credential=AzureKeyCredential(env.face.api_key.get_secret_value())
    )


def get_face_admin_client(env: CobraEnvironment) -> FaceAdministrationClient:
    if not AZURE_AI_VISION_AVAILABLE:
        raise RuntimeError("Azure AI Vision SDK is not installed. Cannot create FaceAdministrationClient.")
...
def ensure_person_group_exists(person_group_id: str, env: CobraEnvironment) -> bool:
    if not AZURE_AI_VISION_AVAILABLE:
        raise RuntimeError("Azure AI Vision SDK is not installed. Cannot manage person groups.")
    admin_client = get_face_admin_client(env)
    try:
        admin_client.get_person_group(person_group_id)
        logger.info(f"Person group '{person_group_id}' already exists.")
        return True
    except Exception as e:
        if "PersonGroupNotFound" in str(e):
            logger.info(f"Person group '{person_group_id}' not found. Creating it...")
            admin_client.create_person_group(person_group_id, recognition_model=FaceRecognitionModel.FACE_RECOGNITION_V5)
            return True
        else:
            logger.error(f"An error occurred while checking for person group '{person_group_id}': {e}")
            return False


def analyze_faces(image_path: str, env: CobraEnvironment, person_group_id: Optional[str] = None) -> List[Dict[str, Any]]:
    if not AZURE_AI_VISION_AVAILABLE:
        raise RuntimeError("Azure AI Vision SDK is not installed. Cannot analyze faces.")
    ...
    if person_group_id and results:
...
def process_frame_with_faces(image_path: str, env: CobraEnvironment, person_group_id: Optional[str] = None) -> Dict[str, Any]:
    if not AZURE_AI_VISION_AVAILABLE:
        raise RuntimeError("Azure AI Vision SDK is not installed. Cannot process frame with faces.")
    # This is a placeholder for a more complex function.
    # For now, it just calls analyze_faces and returns the result in a structured way.
    face_analysis_results = analyze_faces(image_path, env, person_group_id)
    
    result = {
        "frame_path": image_path,
        "faces": face_analysis_results,
        "identified_people": []
    }
    
    # Create a list of identified people for easier access
    if face_analysis_results:
        for face in face_analysis_results:
