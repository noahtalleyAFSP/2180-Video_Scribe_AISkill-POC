...
def generate_batch_transcript(
    ...
    if enable_language_identification:
        ...
        # Per API, still include a primary 'locale' alongside languageIdentification
...
def prepare_outputs_directory(
    ...
    *,
    ...
    """
    Creates and returns a NEW, unique run directory.
    The new directory will be created inside `output_directory` if provided,
    otherwise in the current working directory.
    """
    base_path = Path(output_directory).resolve() if output_directory else Path.cwd()

    # Always generate a unique name for the run subdirectory
    ...
    # The final path for this specific run
    final_run_path = base_path / dir_name

    # Now, check for existence and handle overwrite for the FINAL path
    if final_run_path.exists():
        if overwrite_output:
            _LOG.warning("Overwriting existing run dir %s", final_run_path)
            shutil.rmtree(final_run_path)
        else:
            raise FileExistsError(
                f"Output directory already exists: {final_run_path}.\n"
                "Either delete it manually or pass --overwrite-output."
            )

    final_run_path.mkdir(parents=True, exist_ok=True) # exist_ok is safe because we just deleted it if it existed
    return str(final_run_path)


# --- All Face API functions removed ---
...
def estimate_image_tokens(messages: list[dict]) -> int:
    ...
    prompt. If they are missing we assume one 512^2 tile.
    """
    total = 0
    for m in messages:
