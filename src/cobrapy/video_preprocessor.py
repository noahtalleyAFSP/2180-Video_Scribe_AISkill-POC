...
from .analysis.languageIdentifier import identify_languages_from_video, reduce_to_language_code  # Language detection
from pathlib import Path  # New import
from .analysis.languageIdentifier import identify_languages_from_video, reduce_to_language_code  # Language detection
...
class VideoPreProcessor:
    ...
    def preprocess_video(
        ...
        # --- Language Identification (Stage 0) ---
        if self.manifest.source_video.audio_found and enable_language_identification:
            try:
                logger.info("--- Starting Language Identification ---")
                lid_res = identify_languages_from_video(Path(self.manifest.source_video.path))
                lang_code = reduce_to_language_code(lid_res)
                self.manifest.source_video.language_code = lang_code
                
                # Store full detection result for downstream use/debugging, converting dataclass to dict
                if hasattr(lid_res, '__dict__'):
                    self.manifest.source_video.language_detection = lid_res.__dict__
                
                logger.info(f"Language identification completed. Detected language_code='{lang_code}'")
                logger.info("--- Finished Language Identification ---")
            except Exception as e:
                logger.warning(f"Language identification failed: {e}. Proceeding with default language settings.")
        # --- End Language Identification ---
        ...
        if enable_language_identification:
            try:
                ...
                lang_code = reduce_to_language_code(lid_res)
                self.manifest.source_video.language_code = lang_code
                # Store full detection result for downstream use/debugging
                try:
                    detection_dict = lid_res.__dict__ if hasattr(lid_res, "__dict__") else {}
                except Exception:
                    detection_dict = {}
                self.manifest.source_video.language_detection = detection_dict
                logger.info(f"Language identification completed. Detected language_code={lang_code}")
            except Exception as e:
        ...
        # --- NEW: Build and save per-segment transcription mapping ---
        if self.manifest.audio_transcription and self.manifest.segments:
            per_segment_trans = {}
            for segment in self.manifest.segments:
                seg_text = parse_transcript(self.manifest.audio_transcription, segment.start_time, segment.end_time)
                per_segment_trans[segment.segment_name] = seg_text
            # Save as JSON for debugging and downstream use
            per_seg_path = os.path.join(asset_dir_path, "per_segment_transcription.json")
            with open(per_seg_path, "w", encoding="utf-8") as f:
                import json
                json.dump(per_segment_trans, f, indent=2, ensure_ascii=False)
            logger.info(f"Saved per-segment transcription mapping to {per_seg_path}")
        # --- END NEW ---
    ...
    def _extract_audio_and_transcribe(self, max_workers: int, enable_language_identification: bool = False):
        ...
        try:
            ...
            logger.info(f"({get_elapsed_time(start_t)}s) Extracting full audio to {local_audio_path}...")
            ...
            logger.info(f"({get_elapsed_time(start_t)}s) Audio extracted ({self.manifest.source_audio.file_size_mb:.2f} MB).")
            ...
            if not blob_config or not blob_config.account_name or not blob_config.container_name or \
               (not blob_config.connection_string and not blob_config.sas_token):
                logger.warning(f"({get_elapsed_time(start_t)}s) Blob Storage not configured. Skipping upload and Batch Transcription.")
                self.manifest.audio_transcription = None
                return

            blob_name_upload = f"audio_uploads/{base_audio_name}"
            audio_blob_sas_url = upload_blob(
                local_file_path=local_audio_path,
                blob_name=blob_name_upload,
                env=self.env,
                overwrite=True,
                read_permission_hours=48
            )

            if not audio_blob_sas_url:
                logger.error(f"({get_elapsed_time(start_t)}s) Failed to upload audio to Blob Storage. Skipping transcription.")
                self.manifest.audio_transcription = None
                return

            # --- 3. Call Batch Transcription ---
            logger.info(f"({get_elapsed_time(start_t)}s) Submitting Batch Transcription job...")
            
            # --- MODIFIED: Use detected language for transcription ---
            candidate_locales = ["en-US"] # Default
            detected_lang = self.manifest.source_video.language_code
            if detected_lang:
                lang_map = {"en": "en-US", "fr": "fr-FR", "ar": "ar-SA", "ary": "ar-MA"}
                if detected_lang in lang_map:
                    candidate_locales = [lang_map[detected_lang]]
                    logger.info(f"Using detected language '{detected_lang}' for transcription. Setting locale to: {candidate_locales[0]}")
                else:
                    logger.warning(f"Detected language '{detected_lang}' has no mapped locale. Defaulting to {candidate_locales[0]}")
            # --- END MODIFIED ---

            # If we already detected the language, skip Azure auto-LID to avoid 400 errors.
            use_auto_lid = enable_language_identification and not detected_lang
            transcript_result = generate_batch_transcript(
                 audio_blob_sas_url=audio_blob_sas_url,
                 env=self.env,
                 candidate_locales=candidate_locales,
                 enable_language_identification=use_auto_lid
            )
            self.manifest.audio_transcription = transcript_result
            if transcript_result:
                 logger.info(f"({get_elapsed_time(start_t)}s) Batch transcription job finished successfully.")
            else:
                 logger.warning(f"({get_elapsed_time(start_t)}s) Batch transcription job failed or returned no result.")

        except Exception as e:
            logger.error(f"({get_elapsed_time(start_t)}s) ERROR during audio extraction or transcription pipeline: {e}")
            self.manifest.audio_transcription = None
        finally:
            # Optional: Clean up local audio
            if os.path.exists(local_audio_path):
                 try:
                      # Keeping WAV for inspection, can be enabled for cleanup
                      # os.remove(local_audio_path)
                      pass
                 except Exception as e:
                      logger.warning(f"Could not remove local audio file {local_audio_path}: {e}")
