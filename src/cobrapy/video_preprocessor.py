...
class VideoPreProcessor:
    ...
    def preprocess_video(
        ...
        # Prepare output directory prefix based on segmentation method
        output_directory_prefix = ""
        if use_scene_detection:
            output_directory_prefix += f"scenedetect_t{scene_detection_threshold}_"
        elif use_speech_based_segments:
             output_directory_prefix += f"speechSegs_"
        else:
            # Use segment_length for prefix if not using other methods
            output_directory_prefix += f"{segment_length}sSegs_"
        
        # Consistently add fps to the prefix
        output_directory_prefix += f"{fps:.2f}fps_".replace(".", "_")

        # Now call prepare_outputs_directory with the correct arguments
