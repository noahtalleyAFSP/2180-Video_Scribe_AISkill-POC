...
def run_yolo(video_path: Path, out_dir: Path,
    ...
    print(f"YOLO will run on device: {device}")
    ...
    progress_interval = max(1, total_frames // 100) if total_frames > 0 else 50
    ...
    while True:
        ...
        if frame_idx % progress_interval == 0:
            if total_frames > 0:
                ...
                print(f"YOLO progress: {frame_idx}/{total_frames} frames ({pct:.1f}%) processed…")
            else:
                print(f"YOLO progress: {frame_idx} frames processed…")
